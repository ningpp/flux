...
OUTPUT_DIR = r"D:\models\onnx\GLM-OCR"
...
def _export_llm_with_dtype(model, output_dir: str, filename: str, torch_dtype: torch.dtype):
    ...
    # Export with larger seq_len to enable batch prefill during tracing
    # The model will work for both prefill (multiple tokens) and decode (single token)
    batch_size = 1
    seq_len = 256  # Trace sequence length - enables batch prefill (was 1, which limited to single-token)
    ...
    position_ids = position_ids.unsqueeze(0).unsqueeze(0).expand(3, batch_size, -1).contiguous()
    ...
    try:
        ...
        # Define dynamic dimensions explicitly with proper constraint values - updated for batch prefill support
        batch_dim = Dim("batch", min=1, max=16)
        seq_dim = Dim("seq_len", min=1, max=8192)  # For inputs_embeds - increased from 4096
        total_seq_dim = Dim("total_seq_len", min=1, max=16384)  # For attention_mask - increased from 8192
        past_seq_dim = Dim("past_seq_len", min=0, max=16384)  # For KV cache - increased from 8192
...
def validate_unified_llm(model, output_dir: str):
    ...
    # NOTE: The unified model now supports both prefill (batch tokens) and decode (single token).
    # It was exported with seq_len=256 to enable efficient batch prefill.

    # ===== Test 1: Decode mode with initial KV cache (simulating after prefill) =====
    print("\n" + "-" * 50)
    print("Test 1: Decode Mode (seq_len=1, past_seq_len=64)")
    print("-" * 50)
    print("  Note: This model supports both decode (seq_len=1) and prefill (seq_len>1).")
