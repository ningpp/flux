...
/**
 * DocOrientationClassifyModel 内存泄露 + 性能验证（CPU / GPU 双设备）。
 * <p>
 * 使用长期存活的 MatManager 与 NDManager 反复推理，验证每轮结束后：
 *   - MatManager.trackedMatCount() 回归基线（修复前 processRgb 产生的 CHW Mat 不会释放，
 *     且 Crop.slice 经 IOUtil.close 而非 matManager.release，导致跟踪表无限累积/原生泄露）；
 *   - 每轮使用的 NDManager 子管理器资源计数为 0。
 * 同时统计单次 batch 推理耗时与吞吐。
 * <p>
 * 用法（flux-ocr 项目根目录）：
 *   mvn exec:java "-Dexec.mainClass=io.github.flux.paddle.DocOrientationLeakPerfVerify" "-Dexec.classpathScope=test"
 *   # 可选参数: <imagePath> <iterations> <batchSize> <modelName>
 *   # 默认: D:\tmp\ocr_2025-8-2_22-14-23.jpg 50 1 PP-LCNet_x1_0_doc_ori
 * <p>
 * 默认依次在 CPU(gpuIndex=-1) 与 GPU(gpuIndex=0) 上运行；若某设备不可用则跳过并提示。
 */
public class DocOrientationLeakPerfVerify {

    static final String MODEL_ROOT_DIR = "D:\\models";

    public static void main(String[] args) throws Exception {
        String imagePath = args.length > 0 ? args[0] : "D:\\tmp\\ocr_2025-8-2_22-14-23.jpg";
        int iterations = args.length > 1 ? Integer.parseInt(args[1]) : 50;
        int batchSize = args.length > 2 ? Integer.parseInt(args[2]) : 1;
        String modelName = args.length > 3 ? args[3] : "PP-LCNet_x1_0_doc_ori";
    ...
    private static boolean verifyDevice(int gpuIndex, String imagePath, int iterations,
        ...
        try (var env = OrtEnvironment.getEnvironment()) {
            DocOrientationClassifyModel model;
            try {
                model = new DocOrientationClassifyModel(MODEL_ROOT_DIR, modelName, env, gpuIndex);
            } catch (Exception e) {
