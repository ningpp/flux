...
/**
 * PP-DocLayoutV3 内存泄露 + 性能验证（CPU / GPU 双设备）。
 * <p>
 * 使用长期存活的 MatManager 与 NDManager 反复推理，验证每轮结束后：
 *   - MatManager.trackedMatCount() 回归基线（修复前 ToCHW Mat 会无界累积）；
 *   - 每轮使用的 NDManager 子管理器资源计数为 0（修复前链式算子 / toType 临时 NDArray 会泄漏）。
 * 同时统计单次 batch 推理耗时与吞吐。
 * <p>
 * 用法（flux-ocr 项目根目录）：
 *   mvn exec:java "-Dexec.mainClass=io.github.flux.paddle.PPDocLayoutV3LeakPerfVerify" "-Dexec.classpathScope=test"
 *   # 可选参数: <imagePath> <iterations> <batchSize>
 *   # 默认: D:\tmp\layout.png 50 1
 * <p>
 * 默认依次在 CPU(gpuIndex=-1) 与 GPU(gpuIndex=0) 上运行；若某设备不可用则跳过并提示。
 */
public class PPDocLayoutV3LeakPerfVerify {
     ...
     * DJL 没有公开的 getResourceCount()，但 BaseNDManager 维护了一个
     * protected ConcurrentHashMap&lt;String, AutoCloseable&gt; resources，
     * 所有经由该 manager 创建的 NDArray 都会登记于此，关闭后移除。
     * 因此该 map 的 size 即为“尚未释放的 NDArray 数量”，可直接用于泄露判定。
     * 拿不到时返回 -1（忽略该项断言）。
     */
